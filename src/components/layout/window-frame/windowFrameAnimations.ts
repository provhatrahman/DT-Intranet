...
export function getInitialAnimation(params: {
  ...
  const { shouldAnimateRestore, dockIconOffset, isInitialMount } = params;
...
export function getExitAnimation(params: {
  ...
  // Framer Motion resolves function-valued exit definitions when the exit
  // animation starts, so the dock icon is measured at minimize time — icons
  // shift as apps open/close and the dock re-centers, and the icon may not
  // even exist yet when the window mounts.
