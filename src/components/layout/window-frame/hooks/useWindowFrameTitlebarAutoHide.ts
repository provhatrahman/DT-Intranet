...
export function useWindowFrameTitlebarAutoHide(
  ...
  // When auto-hide gets disabled, force the titlebar visible. The initial
  // state only evaluates once, so without this the titlebar would stay
  // hidden after returning from a view where it had been hidden.
  useEffect(() => {
