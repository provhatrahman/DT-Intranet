 ...
 * `browserReserved` so they only fire inside the Tauri desktop shell, and an
 * optional `webFallback` (an Alt/Option-based combo) is shown/handled on the
 * web instead.
 */
...
export interface ShortcutEnv {
  ...
  /** Whether we're running in the Tauri desktop shell. */
  desktop?: boolean;
}
...
interface ShortcutDef {
  ...
  otherPrimary?: Combo;
  /** Primary combo is reserved by the browser; only usable in the Tauri shell. */
...
/**
 * Resolve the host platform for shortcut purposes, from `navigator` hints.
 * Defaults to "other" (Ctrl) when unknown.
 */
...
function activeCombo(
  def: ShortcutDef,
  { platform, desktop }: Required<ShortcutEnv>
): Combo | null {
  if (def.browserReserved && !desktop) {
 ...
 * environment. Browser-reserved combos only match inside Tauri (or via
