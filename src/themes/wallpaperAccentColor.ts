...
// NOTE(greenroom-phase1): MAIN's `pickPrimaryColor` lives in the iPod
// lyrics-display module, which our fork's iPod app doesn't have (it's an
// older, simpler iPod). `resolveWallpaperAccentFromPalette` below is only
// consumed by the dynamic-wallpaper subsystem (now-playing-cover wallpaper)
// and Control Panels, both explicitly out of scope for this phase, so it's
// stubbed out rather than pulling in an unrelated iPod subsystem port.
// Re-enable by restoring this import once colorUtils.ts (or an equivalent)
// exists in this repo's iPod app.
...
// NOTE(greenroom-phase1): disabled along with the `pickPrimaryColor` import
// above (see comment there). Only the dynamic-wallpaper subsystem called this.
