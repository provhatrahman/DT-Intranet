...
/**
 * Trimmed vs. MAIN: this fork has no dynamic wallpaper subsystem (day/night
 * gradient, weather, now-playing cover/lyrics, shuffle) — every wallpaper here
 * is a static asset or a custom (IndexedDB) upload. So the preview only needs
 * to distinguish "renderable static image" (built-in photo/tile) from
 * "fall back to the plain pinstripe backdrop" (video, custom upload — no
 * cheap synchronous URL to paint into a `background-image`).
 */
