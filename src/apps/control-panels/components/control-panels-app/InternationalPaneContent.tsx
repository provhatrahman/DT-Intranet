...
export function InternationalPaneContent({
  ...
  // GREENROOM (phase 1, English-only): SUPPORTED_LANGUAGES only has "en"
  // right now (src/lib/languageConfig.ts), so LanguageCode is a single-value
  // union. MAIN's ternary chain comparing currentLanguage against other
  // locale literals doesn't typecheck against that narrowed union (no
  // overlap), so this is trimmed to the one supported language. Re-add the
  // other branches here if SUPPORTED_LANGUAGES grows back.
  const languageLabel = t("settings.language.english");
