...
interface IpodArtworkPlaceholderProps {
  ...
  selected?: boolean;
  /** Gradient tile only — use while an image URL exists but bits are still in flight. */
  hideGlyph?: boolean;
  className?: string;
}

/**
 * Empty-state artwork for the modern iPod skin (missing album covers). While a
 * real JPEG is downloading, callers may set `hideGlyph` so only the neutral
 * silver tile appears under a zero-opacity `<img>`.
 */
