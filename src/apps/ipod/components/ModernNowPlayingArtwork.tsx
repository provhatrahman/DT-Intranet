...
import { IpodArtworkPlaceholder } from "./IpodArtworkPlaceholder";
...
/** Left→right perspective (rotate around vertical axis). */
...
/**
 * Modern now-playing album art: 3D-tilted sleeve + mirrored reflection.
 * URLs ping-pong between two fixed `<img>` slots so committing a cross-fade
 * never re-points the displayed bitmap at a freshly reset decode hook (avoids
 * gray flicker).
 */
export function ModernNowPlayingArtwork({
  coverUrl,
}: {
  coverUrl: string | null;
}) {
  const reflectH = MODERN_NOW_PLAYING_ART_PX * MODERN_NOW_PLAYING_REFLECT_RATIO;
  const reflectTargetOpacity = MODERN_NOW_PLAYING_REFLECT_IMG.opacity as number;
  ...
  const fadeCommitTimerRef = useRef<ReturnType<typeof setTimeout> | null>(null);
  const back = 1 - front;
