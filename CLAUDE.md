...
**Authentication:** Google OAuth (Authorization Code + PKCE, server-side token exchange) is **live and enforced in prod** as of 2026-07-20 (frontend build `1a82122`) — see **`AUTH_SETUP.md`** for the full design. **Frontend gate is ON:** `VITE_AUTH_ENABLED=true` is baked into the live build; the desktop requires Google sign-in (`App.tsx` renders `LoginScreen` until authenticated). **Backend enforcement is ON:** `REQUIRE_AUTH=true` on all domain Lambdas (users, artists, bookings, projects, tasks, pitches, payments, analytics, music) — anonymous `/api/*` calls now return `401`, and a valid allowlisted Google `id_token` (`Authorization: Bearer`) is required. **The admin allowlist is no longer merely UI-only** — the backend enforces it too: the allowlist is the `users` table in the prod DB (`DT-Test`), only listed emails can sign in, and `is_admin` derives from `role` (`admin`/`manager`); the frontend allowlist config (`src/config/greenroomAdmins.ts`) now mirrors backend-enforced access rather than being purely cosmetic. `deploy.ps1` **defaults** `VITE_AUTH_ENABLED=true` (gated) — only override to `false` for a deliberately un-gated build. Google `id_token`s expire ~1h; `greenroomFetch` does silent re-auth (`prompt=none` via `useAuthStore.silentReauth`) on `401`. **Rollback:** redeploy the frontend with `VITE_AUTH_ENABLED=false` to un-gate, and flip backend `REQUIRE_AUTH=false` to reopen the API.
