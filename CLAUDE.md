...
**Authentication:** Google OAuth (Authorization Code + PKCE, server-side token exchange) has been **built** end-to-end and verified in local dev — see **`AUTH_SETUP.md`**. It is **OFF by default and not yet enforced in prod**: the frontend gate is gated by `VITE_AUTH_ENABLED` (off) and the backend bearer middleware only enforces when `REQUIRE_AUTH=true` (off). **Until enabled in prod, all gateway routes remain open and writes anonymous, and the frontend "admin" allowlist (`src/config/greenroomAdmins.ts`) is UI-only** — don't treat current prod as access-controlled. Enabling in prod = the checklist in `AUTH_SETUP.md` (build flag, all-domain Lambda redeploy, client secret, gateway CORS, CloudFront `/auth/callback` fallback, allowlist emails, flip `REQUIRE_AUTH`).
